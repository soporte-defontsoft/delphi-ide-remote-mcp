...
| **The IDE's own `GetItCmd`** | `delphi_getit` | The GetIt packages installed in the server's RAD Studio — what the agent has available to program with. List only; installing stays a human decision. |
...
| `delphi_getit` | **What the server has installed to program with**: the GetIt packages of the server's RAD Studio (component libraries, styles, SDK add-ons), listed with the IDE's own `GetItCmd`. Read-only by design — no install/uninstall (that stays a human decision); a missing package is reported with `delphi_report` |
