...
> **Status: stable (1.5.4).** Covered by 82 end-to-end batteries — 2,256 checks — against DelphiLSP 37.0 (RAD Studio 13), and by a full day of real-world field testing by an independent agent using it as a client. A minor version adds tools or capabilities, a patch fixes, and a documented contract that changes is announced in the CHANGELOG first. See [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) and [docs/DELPHILSP-NOTES.md](docs/DELPHILSP-NOTES.md) for the measured research this project is built on, [CHANGELOG.md](CHANGELOG.md) for versions, and [docs/ROADMAP.md](docs/ROADMAP.md) for what is delivered, open, parked or declined.
...
| `delphi_git` | Whitelisted git operations — including **`clone`/`pull`** (bring a whole repo onto the server in one call, jailed) plus status/diff/log/show/branch/switch/merge (always `--ff-only`)/stash (push, pop, list)/add/commit/init/push/tag/config/fetch, and **`worktree`**: another version of the repo next to it (`add` with `path` = a new folder inside your roots and `ref` = a tag, branch or commit, detached), to build and test it and compare - how a remote agent checks an old release without leaving the MCP; `list` shows them, `remove` takes away only those, never with changes or with a link inside. Options that write files or read outside the repo (`--output`, `--no-index`, `-c`…) are refused at the gate |
...
**In a hurry? [QUICKSTART.md](QUICKSTART.md)**: one Windows machine with
...
- **`[Log]`** — the server's log on disk, the **same in every mode** (service, terminal and
  ...
  `delphi_fetch` chunk) is not kept: a mark with its size stays, the rest of the line whole.
  The tray's window shows the latest `LinesPerFile` lines on top. Memory is
  bounded end to end: the buffers cap at 5000 lines (beyond that, lines are counted and
  reported as dropped, never accumulated). Until 1.5.0 only the tray wrote this log: the
  service and the terminal had none.
