...
THE FLOW, and it is the whole trick: `command=screenshot` brings the WHOLE desktop here as a PNG; you LOOK at it, measure the pixel you want, `command=tap` presses exactly there and `command=type` writes text (with `x`,`y` it presses there first - the real gesture is "write this here", and it pays the startup once) - x and y measured *on that screenshot*, because the node converts the screen scale itself. An agent never deals with logical versus physical coordinates: it acts on what it sees. `command=key` presses one key - on a Linux target by its Linux code (evdev, NOT X11 keycodes: Escape 1, Tab 15, Enter 28), on a Windows target by NAME (escape, enter, tab, f4...); the tool reads the profile's platform and refuses the other kind, because a number on Windows would press a different key. `command=windows` shows EVERY window - thumbnails on Linux (the Super key), a list with title and rectangle on Windows - which is how you reach a window another one covers. `command=status` says whether the desktop is reachable and, when it is not, what to ask the operator for. Every answer carries `graphicalEnv`: the session the node ran in.

**How a gesture travels.** On Linux the server drops a `/bin/sh` script in the deploy folder and PAServer runs it (`paclient --put` flag 5); the script completes `DISPLAY` / `WAYLAND_DISPLAY` / `XAUTHORITY` / D-Bus from the user's session when PAServer runs as a service (see `remote-run`). PAServer on Windows executes no scripts - flag 5 is a bare `CreateProcess`, and `paclient` passes no arguments (measured) - so the server ships a tiny native launcher, `node\McpRunJob.exe`, sends it as `run-<job>.exe` next to a three-line job file, and the launcher starts the node unattended, leaves a watcher that writes the exit code, and returns at once (PAServer waits for what it launches, and `paclient` with it). A Windows PAServer must run **inside the user's session**: a Windows service lives in session 0, which has no desktop, and `graphicalEnv` says so.
