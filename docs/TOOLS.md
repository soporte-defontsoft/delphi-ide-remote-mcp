...
| `command` | string | optional | view (default: project summary; section= brings the detail per area) \| add-platform (enable a platform; with `sdk` and/or `profile` it also leaves them set on the project, which is what the IDE's own dialog does: platform + connection + SDK in one gesture) \| remove-platform (disable it again) \| set-output (put every binary under one folder, e.g. Compiled) \| set-version (the project VERSION: the Windows VERSIONINFO numbers and the FileVersion/ProductVersion keys, which have to agree; Android and iOS numbering is not touched) \| set-sdk (the SDK this PROJECT builds a platform with - its own PlatformSDK, which is the IDE's model; \"none\" goes back to the SDK Manager default)  \| set-profile (the PAServer profile this PROJECT deploys and runs a platform with - its own Profile property; the twin of set-sdk, because adding a target to a project is giving it BOTH) \| add-searchpath (add a unit search path for one platform, or for all) \| remove-searchpath (take it out again) \| add-deployfile (ship an extra file with the build on one platform: a component's runtime .so/.dll/.dylib) \| remove-deployfile (take it out again) \| add-unit (register an existing .pas in the project: uses of the .dpr, CreateForm for forms, DCCReference of the .dproj) \| remove-unit (take it out of the project; the file stays on disk) |
...
| `active` | string | optional | get-sdk: make this the ACTIVE SDK of the platform — the bold entry of the IDE's SDK Manager, the one a project builds with when it declares none (the dialog's "Make the selected SDK active"). Default: only when the platform has none yet; `si` forces it, `no` leaves it alone |
...
The run script also takes care of the **graphical environment** of the target: a PAServer started as a service (the normal setup) is born outside the desktop session and a program with a window dies at once (GTK, exit 134, "Can't create a GtkStyleContext without a display connection"). Before launching, the script completes only what is MISSING - `XDG_RUNTIME_DIR`, the session D-Bus, `WAYLAND_DISPLAY`, `DISPLAY` and the newest `XAUTHORITY` - from the session of the SAME user PAServer runs as, and the answer says which case it met in `graphicalEnv`: inherited (PAServer in the session, nothing added), completed (lists the variables), or no graphical session at all (a console program runs anyway; a GUI needs a session open for that user). Measured on a Fedora with PAServer as a systemd service and on a Zorin with PAServer in the session (2026-09-22).
...
The Linux desktop of a target, the way `delphi_adb` gives you an Android one: SEE the screen and ACT on it. The machine hangs off a PAServer profile (the same profiles `delphi_paserver` builds and deploys with) and runs a small Delphi node **this server deploys and updates by itself** — nothing else is installed on it: the node leans only on libraries the GNOME desktop already ships. With `project` empty (the normal case) the node BUNDLED with the server (`node\McpDesktopNode`, part of the distribution) is pushed to the target on first use and refreshed whenever its version stamp stops matching: the server compares the target's `node.ver` (the binary's SHA-256) against its bundled copy once per profile and session, so an updated server heals every already-provisioned Linux on the next gesture, and nothing is ever compiled for it. Since v0.99 the distribution carries BOTH builds of the node and the target gets the one that fits it, read from the profile's own platform: the ELF for a Linux and the `.exe` for a Windows target. **The Windows-target path is written but not measured** — it needs a machine running PAServer for Windows, which we do not have yet; the Linux one is exercised daily. The node's sources live in this repo (`src_desktop_node/`). **GNOME only for now** (measured on Zorin 18 and Fedora): portal capture, Mutter scale, Super overview - other desktops are not supported yet.
...
The target needs a graphical session open - a headless box has nothing to show - for the SAME user PAServer runs as. PAServer itself may run inside that session or as a (user) service: since 1.0.16 the run script completes `DISPLAY` / `WAYLAND_DISPLAY` / `XAUTHORITY` / D-Bus from that session whenever PAServer did not inherit them, and every `remote-run` answer says which case it met (`graphicalEnv`). Measured 2026-09-22 on a Fedora with PAServer as a systemd service, where GalateaFMX used to abort with "Can't create a GtkStyleContext without a display connection" while the node's captures worked (the portal only needs D-Bus). A PAServer running as ANOTHER user (root) sees no session at all. No deploy step is needed for the node itself; `delphi_build target=Deploy` is only for when you develop the NODE and want your own build on the target (then pass `project`).
