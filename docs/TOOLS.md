...
| `edits` | string | optional | Several edits on THIS SAME file, in one call and ALL OR NOTHING: a JSON array `[{"old":"...","new":"...","atline":12},...]` applied IN ORDER. An anchor may be ONE full line or a contiguous BLOCK of lines, matched whole and in order; `"occurrence": 1, 2...` picks which one when it repeats (better than `atline` inside a batch: line numbers MOVE as earlier entries add or remove lines); `"delete": true` removes the line; `"toline": N` makes that entry a range. Ranges are dragged too, so an earlier entry that adds lines corrects the later `toline` by itself. A field that is not one of old/new/atline/toline/delete/occurrence is REFUSED, not ignored. If one entry fails the file goes back byte for byte and the answer names it. Changes across SEVERAL files are delphi_changeset. When `edits` is given, old/new/atline/delete/toline are ignored |
