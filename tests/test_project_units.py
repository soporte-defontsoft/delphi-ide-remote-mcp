...
check('copy unit: COPIADO y el origen sigue', out.startswith('COPIADO') and os.path.exists(os.path.join(cdir, 'UOrig.pas')), out[:300])
...
check('copy unit: ningun proyecto reapuntado, y la respuesta lo dice', 'REAPUNTADA' not in out and 'add-unit' in out and 'copia de seguridad' not in out, out[:400])
out = call('delphi_move', {"path": cdir, "dest": os.path.join(BASE, 'Copia2'), "copy": True})
check('copy carpeta: COPIADO, las dos existen, sin la papelera del origen', out.startswith('COPIADO') and os.path.exists(os.path.join(BASE, 'Copia2', 'UCopia.pas')) and os.path.isdir(cdir) and not os.path.isdir(os.path.join(BASE, 'Copia2', '__delphi-patch')), out[:300])
out = call('delphi_move', {"path": VDIR, "dest": os.path.join(BASE, 'AppCopia'), "copy": True})
check('copy carpeta con .dproj: RECHAZADO (un proyecto nunca en dos sitios)', out.startswith('RECHAZADO') and 'dos sitios' in out and not os.path.exists(os.path.join(BASE, 'AppCopia')), out[:300])
out = call('delphi_move', {"path": os.path.join(cdir, 'UOrig.pas'), "dest": os.path.join(cdir, 'UCopia.pas'), "copy": True})
