...
- `run_all.py` — runs every `test_*.py` against a **copy** of the exe (so
  ...
  and prints one summary line: `49 baterias | N checks OK | N fallos`.
...
- `paclient_stub.py` — a fake `paclient.exe` (via `DELPHI_MCP_PACLIENT`) that
  executes the flag-5 launch scripts for real, so `remote-run` is tested
  end-to-end without a live PAServer.
