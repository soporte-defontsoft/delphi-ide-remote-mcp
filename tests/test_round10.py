...
BASE = mc.carpeta('round10')
EXE = mc.copia_exe(BASE)


def spawn(extra_env=None):
    # sin las DELPHI_MCP_* de quien la lanza (GIT_REMOTES y REMOTE_HOSTS
    # incluidas: R1 mide la puerta con la lista vacia)
    env = mc.entorno({'DELPHI_MCP_ROOTS': BASE, 'DELPHI_MCP_ALLOW_TESTS': '1'})
    env.update(extra_env or {})
    # hasta 900 s por llamada: compila y ejecuta tests
    return mc.Stdio(EXE, env, nombre='r10', t=900)
...
A.call('delphi_changeset', {'command': 'stage', 'id': CID2, 'kind': 'create',
                             'path': NUEVO,
                             'content': 'unit Nuevo;\ninterface\nimplementation\nend.\n'})
pv = J(A.call('delphi_changeset', {'command': 'preview', 'id': CID2}))
...
r = A.call('delphi_changeset', {'command': 'commit', 'id': CID2})
...
j = J(A.call('delphi_list', {'root': os.path.join(PRJ, 'Win64'), 'pattern': '*'}))
check('P1 nombrar Win64 como root SI lista lo que hay debajo',
      j.get('total', 0) >= 1, str(j)[:250])
j = J(A.call('delphi_list', {'root': PRJ}))
...
open(os.path.join(T, 'MiTest.dpr'), 'w', encoding='utf-8-sig', newline='\r\n').write(
    ...
    "  Halt(1);\nend.\n")
j = J(A.call('delphi_test', {'command': 'run', 'project': os.path.join(T, 'MiTest.dproj')},
           t=900))
...
open(UPAS, 'w', encoding='utf-8', newline='\r\n').write(
    ...
    'implementation\n\nfunction Doble(A: Integer): Integer;\nbegin\n'
    '    Result := A * 2;\nend;\n\n'
    # dos apariciones en UNA linea sangrada: lo que prometen los dos D1
    'function Cuadruple(A: Integer): Integer;\nbegin\n'
    '    Result := Doble(Doble(A));\nend;\n\nend.\n')
