...
def spawn(extra_env=None):
    env = {'DELPHI_MCP_ROOTS': BASE, 'DELPHI_MCP_VAULT_PATH': VAULT,
           'DELPHI_MCP_VAULT_READONLY': '0'}
    env.update(extra_env or {})
    # "r8": la identidad que mira #9; plazo de 600 s: aqui se compila
    return mc.Stdio(EXE, mc.entorno(env), nombre='r8', t=600)


def raw_result(srv, name, args, t=None):
    """The whole tools/call result as it travels: what the client really sees."""
    r = srv.call_msg(name, args, t)
    return json.dumps(r) if r else '(timeout)'


def J(t):
    # json.loads ESTRICTO (no mc.como_json): #9 mide que la respuesta siga
    # siendo JSON valido
    try:
        return json.loads(t)
    except Exception:
...
CONT = 'unit ULlena;\r\n\r\ninterface\r\n\r\nfunction Doble(A: Integer): Integer;\r\n\r\n' \
       'implementation\r\n\r\nfunction Doble(A: Integer): Integer;\r\nbegin\r\n' \
       '  Result := A * 2;\r\nend;\r\n\r\nend.\r\n'
r = A.call('delphi_create', {'kind': 'unit', 'name': 'ULlena', 'content': CONT,
                              'project': SANODPROJ})
...
open(os.path.join(SOLO, 'UAlgo.pas'), 'w', encoding='utf-8').write(
    'unit UAlgo;\r\ninterface\r\nimplementation\r\nend.\r\n')
j = J(A.call('delphi_config', {'project': SOLODPR}))
...
j = J(A.call('delphi_designer', {'command': 'prop', 'class': 'TButton', 'prop': 'Align'}))
...
open(DFM, 'w', encoding='utf-8').write(
    ...
    "  end\r\n"
    "end\r\n")
r = A.call('delphi_designer', {'command': 'lint', 'path': DFM})
...
j = J(A.call('delphi_help', {'name': 'delphi_changeset'}))
...
r = A.call('delphi_help', {'command': 'volar'})
